...
if (document.querySelector('h1.nologin') !== null) {
    ...
    const fumenFilter = (stats, diff, medal, rank, score, version, lv, lv_type) => {
        ...
        const percentile_score_data_raw2 = alasql(`MATRIX OF
SELECT TBL1.[0] AS [0],
TBL1.[1] AS [1],
TBL1.[2] AS [2],
TBL1.[3] AS [3],
...
INNER JOIN ? AS TBL4 ON TBL4.[0] = TBL1.[0]`;
    ...
    // filter
        ...
        // version除去/列番号変更
        const result = res2.map(a => [a[0], a[1], a[2], a[3], a[10], a[11], a[6], a[7], a[4], a[8], a[9]]);
        return result;
    };
    ...
    const updateGrid = data => {
        ...
        const columns_setting = [
            ...
            },
            {
                ...
                name: 'diff',
                formatter: (_, row) => {
                    return row.cells[7].data <= 0 ? row.cells[7].data : ('+' + row.cells[7].data);
                },
                attributes: (cell) => {
                    if (cell === null) {
                        return undefined;
            ...
            },
            {
                ...
                name: 'score',
                formatter: (_, row) => {
                    return row.cells[8].data === -2 ? '-' : row.cells[8].data;
                },
                attributes: (cell) => {
                    if (cell === null) {
                        return undefined;
    ...
    const updateGrid2 = () => {
        ...
        const key_stats = Object.keys(STATS_DATA).filter((key) => {
            return STATS_DATA[key] === val;
        })[0];
